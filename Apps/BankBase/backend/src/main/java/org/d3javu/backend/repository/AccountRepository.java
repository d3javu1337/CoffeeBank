...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    @Query(value = "select count(*) > 0 from personal_account a where a.id= :accountId and a.client_id= :clientId",
            nativeQuery = true)
    Boolean isClientOwnsAccount(Long clientId, Long accountId);

    @Query(value = "select a.id from personal_account a where a.client_id= :clientId",
            nativeQuery = true)
